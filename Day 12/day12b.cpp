...
int main()
{
	...
	while(walk != NULL)
	{
		if(findNode(list, walk->id) == false)
		{
			node* innerList = NULL;
			findChildren(head, walk->id, innerList, programCount);
			groups++;

			node* listWalk = innerList;
			while(listWalk != NULL)
			{
				addNode(list, listWalk->data);
